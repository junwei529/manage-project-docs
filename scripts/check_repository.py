...
EXPECTED_SOURCE_MAPPING_SHA256 = 'a155bcc2ca306fef2f7bc3b43a654c9967c137a95db40541956cc08f945fc2ba'
...
def file_uri_private_locator_matrix() -> list[tuple[str, str, str]]:
    ...
    cases = [
        ...
        (
            "private-file-uri-drive-authority-upper",
            "file:" + forward * 2 + "C:" + forward + forward.join(("Users", ascii_space_profile, "project")),
            profile_label,
        ),
        (
            "private-file-uri-drive-authority-lower-end",
            "file:" + forward * 2 + "c:" + forward + forward.join(("Users", unicode_profile)),
            profile_label,
        ),
        (
            "private-file-uri-encoded-drive-authority",
            "file:" + forward * 2 + quote("C:", safe="") + forward + forward.join(("Users", quote(ascii_space_profile, safe=""))),
            profile_label,
        ),
        (
        ...
        (
            "private-file-uri-unicode-unc-authority",
            "file:" + forward * 2 + unicode_server + forward + forward.join((unicode_share, "project")),
            unc_label,
        ),
        (
            "private-file-uri-encoded-unicode-unc-authority",
            "file:" + forward * 2 + quote(unicode_server, safe="") + forward + forward.join((quote(unicode_share, safe=""), "project")),
            unc_label,
        ),
        (
            "private-file-uri-empty-authority-unc-path",
            "file:" + forward * 4 + forward.join((unicode_server, "team" + " " + "share", "project")),
            unc_label,
        ),
        (
...
def ambiguous_locator_matrix() -> list[tuple[str, str, str]]:
    ...
    return [
        (
            "ambiguous-file-uri-device-query-drive",
            "file:" + forward * 2 + "?" + forward + profile_like_path,
            ambiguous_file,
        ),
        (
            "ambiguous-file-uri-device-query-unc",
            "file:" + forward * 2 + "?" + forward + forward.join(("UNC", "server", "share")),
            ambiguous_file,
        ),
        ...
        (
            "ambiguous-file-uri-double-encoded-drive",
            "file:" + forward * 2 + quote(quote(profile_like_path, safe=""), safe=""),
            ambiguous_file,
        ),
